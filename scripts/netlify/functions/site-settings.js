...
export const handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    const GATHERING_COLS = ['reg_gathering_closed', 'gathering_max_participants', 'gathering_venue_payment_enabled', 'gathering_email_provider', 'gathering_food_code_in_email_enabled'];
    ...
    const settings = {
      ...
      // Round 112: raw manual switch, defaults false to match the column's
      // own DEFAULT — controls whether the food QR/passcode block is
      // auto-included in the registration-received and payment-confirmed
      // emails. Never gates the standalone food-code email/blast, which
      // always includes it.
      gathering_food_code_in_email_enabled: !!data?.gathering_food_code_in_email_enabled,
  ...
  if (event.httpMethod === 'PUT') {
    ...
    try {
      ...
      for (const key of ['reg_ph_closed', 'reg_intl_closed', 'ph_pay_later_enabled', 'merch_preorder_closed', 'reg_gathering_closed', 'gathering_venue_payment_enabled', 'gathering_food_code_in_email_enabled', ...NOTIFICATION_KEYS]) {
