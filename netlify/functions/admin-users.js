  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      const { data: newAdmin, error: insertErr } = await supabase.from('admins').insert({
        ...
        name,
        password_hash: hash,
        permissions: permissions || { receive_updates: true, verify_payment: false, merch_order_notify: false, gathering_email_updates: false, manage_admins: false, manage_churches: false },
